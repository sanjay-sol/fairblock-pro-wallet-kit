...
export default function Dashboard() {
  ...
  const {
    treasury, balances, nativeBalance, symbol, nativeSymbol, network, txs, analytics,
    busy, sessionActive, activateTreasury, depositToConfidential, refreshBalances,
  } = useOrg();
  ...
  return (
    <div className="page">
      {/* funding nudge — a fresh wallet starts empty */}
      {needsFunding && (
      ...
      <div className="card-row cols-2" style={{ marginBottom: 16 }}>
        <div className="card">
          <div className="between">
            <h3>Treasury</h3>
            <span className={`badge ${treasury.activated ? "ok" : "warn"}`}>
              {treasury.activated ? "confidential ready" : "not activated"}
            </span>
          </div>
          <div className="flex" style={{ margin: "10px 0 16px" }}>
            <span className="mono muted">{short(treasury.address)}</span>
            <CopyBtn value={treasury.address} label="" />
          </div>
          ...
          <div className="stats" style={{ gridTemplateColumns: "1fr 1fr", marginTop: 10 }}>
            ...
            <Stat k="Network" icon="settings">{network?.shortName}</Stat>
          </div>
          {Number(balances.confidential.pending) > 0 && (
            <p className="hint" style={{ marginTop: 10 }}>Pending (unapplied): {fmtAmount(balances.confidential.pending)} {symbol} — applied automatically on next op.</p>
          )}

          <div className="flex wrap" style={{ marginTop: 16 }}>
            <button className="btn sm" onClick={() => nav("/fund")}><Icon.receive size={14} /> Deposit</button>
            {!treasury.activated && (
              <AsyncButton className="btn sm primary" onClick={activateTreasury} disabled={busy} loadingText="Deriving…">
                <Icon.shield size={14} /> Derive confidential keys
              </AsyncButton>
            )}
        ...
        <div className="card">
          ...
          <div className="inline">
            ...
            <AsyncButton
              ...
              style={{ flex: "0 0 auto" }}
              disabled={busy || !treasury.activated || !(Number(depAmt) > 0)}
              onClick={() => depositToConfidential(depAmt)}
              loadingText="Depositing…"
            >
              <Icon.download size={15} /> Deposit
            </AsyncButton>
          </div>
          {!treasury.activated && <p className="hint" style={{ marginTop: 10 }}>Derive your confidential keys first.</p>}
          {treasury.activated && !hasToken && !hasConfidential && (
            <p className="hint" style={{ marginTop: 10 }}>Get some {symbol} first — see <a className="link" onClick={() => nav("/fund")}>Fund wallet</a>.</p>
          )}
          {treasury.activated && hasToken && !hasConfidential && (
            <p className="hint" style={{ marginTop: 10 }}>Deposit some {symbol} to start making confidential payouts.</p>
          )}
          {!sessionActive && (
            <p className="hint" style={{ marginTop: 10 }}>Tip: <b>Create a session</b> once (top-right) and sign seamlessly — no repeated passkey prompts. <a href="https://docs.turnkey.com/authentication/sessions" target="_blank" rel="noreferrer" style={{ color: "var(--brand-soft)", textDecoration: "underline" }}>About sessions ↗</a></p>
          )}
        </div>
      </div>
      ...
      <div className="stats" style={{ marginBottom: 16 }}>
        ...
        <Stat k="Pending approvals" icon="pending">{txs.filter((t) => t.status === "pending").length}</Stat>
      </div>
      ...
      <div className="card-row cols-2" style={{ marginBottom: 16 }}>
        <button className="card" style={{ textAlign: "left", cursor: "pointer" }} onClick={() => nav("/single")}>
          ...
          <p className="csub" style={{ margin: "8px 0 0" }}>Send one confidential payment to a recipient's wallet.</p>
        </button>
      ...
      <div className="card">
        ...
        {recent.length === 0 ? (
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>Type</th><th>Recipient</th><th>Amount</th><th>Status</th><th>When</th></tr>
              </thead>
              <tbody>
                {recent.map((t) => (
                  <tr key={t.id}>
                    <td><KindBadge kind={t.kind} /></td>
                    <td>{t.recipientLabel || <span className="mono">{short(t.to)}</span>}</td>
